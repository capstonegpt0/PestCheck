...
const Detection = ({ user, onLogout }) => {
  ...
  const handleDetect = async () => {
    ...
    try {
      ...
      alert(error.response?.data?.error || error.message || 'Failed to detect pest. Please try again.');
    } finally {
      setLoading(false);
    }
  };
              ...
              {preview && !result && (
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className
                    ="col-span-2 bg-green-600 text-white py-4 rounded-lg hover:bg-green-700 transition-colors font-semibold flex items-center justify-center disabled:bg-gray-400 disabled:cursor-not-allowed"
>
{loading ? (
<>
